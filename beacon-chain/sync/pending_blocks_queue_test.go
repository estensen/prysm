...
func TestRegularSyncBeaconBlockSubscriber_ProcessPendingBlocks1(t *testing.T) {
	...
	// Add b2 to the cache
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b2), b2Root))
	...
	// Add b1 to the cache
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b1), b1Root))
	...
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(testutil.NewBeaconBlock()), [32]byte{}))
...
func TestRegularSync_InsertDuplicateBlocks(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b0), b0r))
	require.Equal(t, 1, len(r.pendingBlocksInCache(b0.Block.Slot)), "Block was not added to map")

	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b1), b1r))
...
func TestRegularSyncBeaconBlockSubscriber_ProcessPendingBlocks_2Chains(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b4), b4Root))
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b5), b5Root))
	...
	// Add b3 to the cache
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(b3), b3Root))
	require.NoError(t, r.cfg.DB.SaveBlock(context.Background(), wrapper.WrappedPhase0SignedBeaconBlock(b3)))
...
func TestService_sortedPendingSlots(t *testing.T) {
	...
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot}})), [32]byte{1}))
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot - 3}})), [32]byte{2}))
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot - 5}})), [32]byte{3}))
	require.NoError(t, r.insertBlockToPendingQueue(wrapper.WrappedPhase0SignedBeaconBlock(testutil.HydrateSignedBeaconBlock(&ethpb.SignedBeaconBlock{Block: &ethpb.BeaconBlock{Slot: lastSlot - 2}})), [32]byte{4}))
